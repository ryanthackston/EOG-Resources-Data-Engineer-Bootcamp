...
CREATE OR REPLACE PACKAGE EMPLOYEE_PKG as 
    ...
    PROCEDURE Get_Employee (
        p_employee_id in NUMBER, 
        employee_data OUT  SYS_REFCURSOR
    );
...
CREATE OR REPLACE PACKAGE BODY EMPLOYEE_PKG as 
    ...
    PROCEDURE Save_Employee_JSON (
        ...
        BEGIN
            ...
            l_hire_date := TO_DATE(l_json_obj.get_String('hire_date'), 'YYYY-MM-DD');
    ...
    --             -- Create a JSON object with all columns of the Employees table
    --             l_json_obj := JSON_OBJECT_T();
    --             l_json_obj.put('employee_id', l_employee_id);
    --             l_json_obj.put('first_name', l_first_name);
    --             l_json_obj.put('last_name', l_last_name);
    --             l_json_obj.put('email', l_email);
    --             l_json_obj.put('phone', l_phone);
    --             l_json_obj.put('hire_date', TO_CHAR(l_hire_date, 'YYYY-MM-DD'));  -- Format date as a string
    --             l_json_obj.put('manager_id', l_manager_id);
    --             l_json_obj.put('job_title', l_job_title);
    --             l_json_obj.put('is_active_fl', l_is_active_fl);

    --             -- Open a cursor to return the JSON object as a result
    --             OPEN p_json_cursor FOR
    --                 SELECT l_json_obj.to_clob() AS employee_json FROM dual;

    --         EXCEPTION
    --             WHEN NO_DATA_FOUND THEN
    --                 -- If no employee found, return an empty JSON object with a message
    --                 l_json_obj := JSON_OBJECT_T();
    --                 l_json_obj.put('message', 'Employee not found');
    --                 OPEN p_json_cursor FOR
    --                     SELECT l_json_obj.to_clob() AS employee_json FROM dual;
    --             WHEN OTHERS THEN
    ...
    --             -- Construct JSON in CLOB format
    --             p_rec := '{"employee_id": ' || l_employee_id ||
    --                     ', "first_name": "' || l_first_name || '"' ||
    --                     ', "last_name": "' || l_last_name || '"' ||
    --                     ', "email": "' || l_email || '"' ||
    --                     ', "phone": "' || l_phone || '"' ||
    ...
    --             WHEN NO_DATA_FOUND THEN
    --                 -- If no employee found, return a JSON object with a message
    --                 p_rec := '{"message": "Employee not found"}';
    --             WHEN OTHERS THEN
    ...
    --         -- Helper procedure to append text to the output CLOB
    --         PROCEDURE append_to_clob(p_clob IN OUT CLOB, p_text IN VARCHAR2) IS
    --         BEGIN
    --             DBMS_LOB.WRITEAPPEND(p_clob, LENGTH(p_text), p_text);
    --         END append_to_clob;
    ...
    --     BEGIN
    --         -- Initialize the output CLOB for status messages
    --         DBMS_LOB.CREATETEMPORARY(p_rec_data, TRUE);
    --         DBMS_LOB.TRIM(p_rec_data, 0);  -- Clear the CLOB in case it has any existing data

    --         -- Attempt to parse the JSON input into a JSON array
    --         BEGIN
    --             l_json_array := JSON_ARRAY_T.parse(p_json_data);
    --         EXCEPTION
    --             WHEN OTHERS THEN
    --                 append_to_clob(p_rec_data, 'Error: Invalid JSON array format in input data.' || CHR(10));
    --                 RETURN;
    --         END;

    --         -- Get the number of employee records in the JSON array
    --         l_count := l_json_array.get_size;

    --         -- Loop through each element in the JSON array
    --         FOR i IN 0 .. l_count - 1 LOOP
    --             BEGIN
    --                 -- Get the JSON object at position i
    --                 l_json_obj := l_json_array.get_Object(i);
    ...
    --                             l_employee_id, l_first_name, l_last_name, l_email, l_phone, l_hire_date, l_manager_id, l_job_title, l_is_active_fl
    --                         );
    --                         append_to_clob(p_rec_data, 'Inserted new employee with ID: ' || TO_CHAR(l_employee_id) || CHR(10));
    --                     ELSE
    --                         append_to_clob(p_rec_data, 'Updated employee with ID: ' || TO_CHAR(l_employee_id) || CHR(10));
    --                     END IF;

    --                 EXCEPTION
    --                     WHEN OTHERS THEN
    --                         append_to_clob(p_rec_data, 'Error processing employee with ID: ' || TO_CHAR(l_employee_id) || ' - ' || SQLERRM || CHR(10));
    --                 END;

    --             EXCEPTION
    --                 WHEN OTHERS THEN
    --                     append_to_clob(p_rec_data, 'Error parsing employee data at index ' || TO_CHAR(i) || ': ' || SQLERRM || CHR(10));
    --             END;
    --         END LOOP;
    ...
    --     EXCEPTION
    --         WHEN OTHERS THEN
    --             -- Rollback the transaction if there’s an error and provide error message in the CLOB
    --             ROLLBACK;
    --             DBMS_LOB.TRIM(p_rec_data, 0);
    --             append_to_clob(p_rec_data, 'Error in save_employees_json: ' || SQLERRM);
    ...
    --     BEGIN
    --         -- Initialize the output CLOB for the JSON array response
    --         DBMS_LOB.CREATETEMPORARY(p_rec_data, TRUE);
    --         DBMS_LOB.TRIM(p_rec_data, 0);  -- Clear the CLOB in case it has any existing data

    --         -- Start JSON array
    --         append_to_clob(p_rec_data, '[');
    ...
    --             )
    --         ) LOOP
    --             -- Insert a comma between JSON objects if this is not the first record
    --             IF NOT l_first_record THEN
    --                 append_to_clob(p_rec_data, ',');
    --             ELSE
    --                 l_first_record := FALSE;
    ...
    --             l_email       := rec.email;
    --             l_phone       := rec.phone;
    --             l_hire_date   := TO_DATE(rec.hire_date, 'YYYY-MM-DD');
    ...
    --                 IF SQL%ROWCOUNT = 0 THEN
    --                     -- If no rows were updated, insert a new record and retrieve the new employee_id
    --                     INSERT INTO Employees (
    --                         employee_id, first_name, last_name, email, phone, hire_date, manager_id, job_title, is_active_fl
    --                     ) VALUES (
    --                         l_employee_id, l_first_name, l_last_name, l_email, l_phone, l_hire_date, l_manager_id, l_job_title, l_is_active_fl
    ...
    --                     -- Append JSON for the newly inserted record
    --                     append_to_clob(p_rec_data, '{"employee_id": ' || TO_CHAR(l_employee_id) || ', "status": "inserted"}');
    --                 ELSE
    --                     -- Append JSON for the updated record
    --                     append_to_clob(p_rec_data, '{"employee_id": ' || TO_CHAR(l_employee_id) || ', "status": "updated"}');
    --                 END IF;

    --             EXCEPTION
    --                 WHEN OTHERS THEN
    --                     -- Append error message for the specific record
    --                     append_to_clob(p_rec_data, '{"employee_id": ' || TO_CHAR(l_employee_id) || ', "status": "error", "message": "' || SQLERRM || '"}');
    --             END;
    --         END LOOP;
    ...
    --             ROLLBACK;
    --             DBMS_LOB.TRIM(p_rec_data, 0);
    --             append_to_clob(p_rec_data, '[{"status": "error", "message": "Error in save_employees_json: ' || SQLERRM || '"}]');
    -- END save_employees_json;
